...
function get_footer() {
  var footer = set_attribute("afooter", "footer-id", "footer");
  var div = set_attribute("footer", "_c", "div");
  var p = set_attribute("c", "ptext", "p");
  p.innerHTML =
    'Copyright @ 2023 <a href="/">asheux.com</a> - All Rights Reserved';
  div.appendChild(p);
...
function blinker() {
  ...
  if (blinker) {
    blinker.style.color = blinker.style.color === "red" ? "white" : "red";
  }
}
...
async function handle_crawl(e) {
  ...
  if (res) {
    ...
    if (!result.length && !errors.length) {
      errors = ["The domain you entered is invalid/or returned no response."];
